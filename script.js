...
let pokemonSeleccionadosIds = new Set();
...
function inicializarPokemons() {
    ...
    pokemons.forEach(pokemon => {
        ...
        pokemonDiv.innerHTML = `
            <img src="${pokemon.img}" alt="${pokemon.name}" class="pokemon-imagen">
            <h2>${pokemon.name}</h2>
            <h3>${pokemon.id}</h3>
            <p class="type">Tipo: ${pokemon.types.join(", ")}</p>

        `;
    
        pokemonDiv.addEventListener('click', () => seleccionarPokemon(pokemon, pokemonDiv));
...
function seleccionarPokemon(pokemon, pokemonDiv) {
    const limiteSeleccion = 2;

    if (pokemonSeleccionados.length < limiteSeleccion) {
        if (!pokemonSeleccionadosIds.has(pokemon.id)) {
            pokemonSeleccionados.push(pokemon);
            pokemonSeleccionadosIds.add(pokemon.id);
            pokemonDiv.classList.add("selected");
        }
    } else {
        if (pokemonSeleccionadosIds.has(pokemon.id)) {
            pokemonSeleccionados = pokemonSeleccionados.filter(p => p.id !== pokemon.id);
            pokemonSeleccionadosIds.delete(pokemon.id);
            pokemonDiv.classList.remove("selected");
        }
    }

    document.getElementById("botonConfirmacion").disabled = pokemonSeleccionados.length !== 2;
}

function seleccionarPokemonAleatorio() {
    limpiarSeleccion();

    while (pokemonSeleccionados.length < 2) {
        const indiceAleatorio = Math.floor(Math.random() * pokemons.length);
        const pokemonAleatorio = pokemons[indiceAleatorio];

        if (!pokemonSeleccionadosIds.has(pokemonAleatorio.id)) {
            pokemonSeleccionados.push(pokemonAleatorio);
            pokemonSeleccionadosIds.add(pokemonAleatorio.id);
            document.getElementById(`pokemon${pokemonAleatorio.id}`).classList.add("selected");
        }
    }

    document.getElementById("botonConfirmacion").disabled = false;
}

function limpiarSeleccion() {
    pokemonSeleccionados = [];
    pokemonSeleccionadosIds.clear();
    const pokemonCards = document.querySelectorAll(".pokemon-card.selected");
    pokemonCards.forEach(card => card.classList.remove("selected"));
}

function confirmarSeleccion() {
    if (pokemonSeleccionados.length === 2) {
        localStorage.setItem("pokemonSeleccionados", JSON.stringify(pokemonSeleccionados));
        window.location.href = "/pantalla_2";
    }
}
...
document.getElementById("botonRandom").addEventListener("click", seleccionarPokemonAleatorio);
document.getElementById("botonConfirmacion").addEventListener("click", confirmarSeleccion);